...
test("exchanges a verifier through the environment-selected Auth endpoint and issues the app cookie", async (t) => {
  ...
  const response = await onRequestPost({
    ...
    request: new Request("https://practiceengine-dev.pages.dev/neon-auth/complete-callback", {
      method: "POST",
      headers: { "content-type": "application/json", origin: "https://practiceengine-dev.pages.dev", "content-length": "35", cookie: "challenge=placeholder" },
  ...
  assert.equal(forwarded.headers.has("content-type"), false);
...
test("rejects a missing verifier without contacting Neon Auth", async (t) => {
  ...
  const response = await onRequestPost({
    ...
    request: new Request("https://practiceengine-dev.pages.dev/neon-auth/complete-callback", {
      method: "POST",
      headers: { "content-type": "application/json", origin: "https://practiceengine-dev.pages.dev" },
      body: "{}",
    }),
  });
...
test("rejects cross-origin verifier submission before contacting the provider", async (t) => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async () => { throw new Error("must not contact provider"); };
  t.after(() => { globalThis.fetch = originalFetch; });
  const response = await onRequestPost({
    request: new Request("https://ledgerly-accounts.pages.dev/neon-auth/complete-callback", {
      method: "POST",
      headers: { "content-type": "application/json", origin: "https://untrusted.example" },
      body: JSON.stringify({ verifier: "test-only-verifier" }),
    }),
  });
  assert.equal(response.status, 403);
});

for (const cookie of ["", "ledgerly_session=; Max-Age=0", "ledgerly_session=expired; Max-Age=0"]) {
  test(`does not report authentication without a new usable session cookie: ${cookie || "absent"}`, async (t) => {
    const originalFetch = globalThis.fetch;
    globalThis.fetch = async () => new Response('{"private":"discard this response"}', {
    ...
    const response = await onRequestPost({
      request: new Request("https://ledgerly-accounts.pages.dev/neon-auth/complete-callback", {
        method: "POST",
        headers: { "content-type": "application/json", origin: "https://ledgerly-accounts.pages.dev" },
        body: JSON.stringify({ verifier: "test-only-verifier" }),
      }),
    });
