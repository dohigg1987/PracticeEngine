...
import {
  ...
  PracticeWorkItem, PracticeWorkStatus, PracticeWorkTemplate, RecurringWorkSchedule,
} from "./api";
...
function WorkOperations(props: Props) {
  const [tab, setTab] = useState<"work" | "recurring">("work");
  return <><TabList selectedValue={tab} onTabSelect={(_, data) => setTab(data.value as typeof tab)}><Tab value="work">Work items</Tab><Tab value="recurring">Recurring work</Tab></TabList>{tab === "work" ? <WorkList {...props} /> : <RecurringWork {...props} />}</>;
}
