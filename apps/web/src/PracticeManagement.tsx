...
import {
  api, ApiContext, AutomationRule, PracticeClientSummary, PracticeReview, PracticeService, PracticeTask,
...
function WorkOperations(props: Props) {
  const [tab, setTab] = useState<"work" | "reviews" | "recurring" | "operations">("work");
...
function PracticeSettings({ context }: Props) {
  const [tab, setTab] = useState<"services" | "templates" | "automation">("services");
  ...
  const load = useCallback(async () => { setLoading(true); setError(""); try { const [serviceData, templateData,automationData] = await Promise.all([api.practiceServices(context), api.practiceWorkTemplates(context),api.automationRules(context)]); setServices(serviceData.items); setTemplates(templateData.items);setAutomation(automationData.items); } catch (e) { setError(errorText(e)); } finally { setLoading(false); } }, [context]);
  useEffect(() => { void load(); }, [load]);
  ...
  async function addAutomation(){try{await api.createAutomationRule(context,{name:"New operational automation",enabled:false,triggerType:"work.created",conditions:[],actions:[{type:"mark_blocked"}],priority:100});await load();}catch(e){setError(errorText(e));}}
  async function toggleAutomation(rule:AutomationRule){try{await api.updateAutomationRule(context,rule.id,{enabled:!rule.enabled});await load();}catch(e){setError(errorText(e));}}
