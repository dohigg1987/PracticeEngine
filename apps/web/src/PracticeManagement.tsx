...
import {
  ...
  PracticeWorkItem, PracticeWorkStatus, PracticeWorkTemplate, RecurrenceExecution, RecurringWorkSchedule,
} from "./api";
...
import DeliveryWorkDetail from "./PracticeWorkDetail";
...
export default function PracticeManagement(props: Props) {
  if (props.view === "work-detail") return <DeliveryWorkDetail {...props} />;
...
function ReviewQueue({ context, onOpenWork }: Props) {
  const [items, setItems] = useState<PracticeReview[]>([]);
  const [loading, setLoading] = useState(true), [error, setError] = useState(""), [status, setStatus] = useState("");
  const load = useCallback(async () => { setLoading(true); setError(""); try { setItems((await api.practiceReviews(context, status)).items); } catch (reason) { setError(errorText(reason)); } finally { setLoading(false); } }, [context, status]);
  useEffect(() => { void load(); }, [load]);
  const visible = status ? items : items.filter(item => !["approved", "completed"].includes(item.status));
  if (loading) return <Loading text="Loading review queue" />;
  return <section className="pm-page"><Head title="Review queue" body="Open the work, resolve review points and record a decision." />
    {error && <Failure message={error} retry={load} />}
    <div className="pm-filters"><Field label="Review status"><Select value={status} onChange={(_, data) => setStatus(data.value)}><option value="">All active reviews</option>{["requested", "in_progress", "changes_requested", "reopened", "approved", "completed"].map(value => <option key={value} value={value}>{label(value)}</option>)}</Select></Field><Button onClick={() => void load()}>Refresh</Button></div>
    <div className="pm-grid-scroll"><Table aria-label="Practice review queue"><TableHeader><TableRow><TableHeaderCell>Work / client</TableHeaderCell><TableHeaderCell>Reviewer</TableHeaderCell><TableHeaderCell>Due</TableHeaderCell><TableHeaderCell>Status</TableHeaderCell><TableHeaderCell>Action</TableHeaderCell></TableRow></TableHeader><TableBody>{visible.map(item => <TableRow key={item.id}><TableCell><span className="pd-primary"><strong>{item.work_title || "Work review"}</strong><small>{item.client_name || "Client"} · {item.stage_name || item.service_name || "Task review"}</small></span></TableCell><TableCell>{item.reviewer_name || "Unassigned"}</TableCell><TableCell>{date(item.due_date)}</TableCell><TableCell><Status value={item.status} /></TableCell><TableCell><Button onClick={() => onOpenWork?.(item.work_item_id)}>Open review</Button></TableCell></TableRow>)}</TableBody></Table></div>
    {!visible.length && !error && <div className="pm-empty"><h3>No reviews in this view</h3><p>Reviews requested from work records will appear here.</p></div>}
  </section>;
}
...
type ClientArea = "overview" | "delivery" | "services" | "collaboration" | "documents" | "economics" | "activity" | "details";
export function clientWorkspaceState(search = "") {
  const parameters = new URLSearchParams(search);
  const requested = parameters.get("area") || "overview";
  return {
    area: (["overview", "delivery", "services", "collaboration", "documents", "economics", "activity", "details"].includes(requested) ? requested : "overview") as ClientArea,
    selected: parameters.get("selected") || "",
    returnPath: parameters.get("return") || "",
  };
}

function ClientSummary({ context, clientId = "", onOpenWork, onOpenLedgerly, onBack, routeSearch, onNavigate }: Props) {
  const workspace = useMemo(() => clientWorkspaceState(routeSearch), [routeSearch]);
  ...
  const [collaborationTab, setCollaborationTab] = useState<"requests" | "messages">("requests");
  const addWorkFocusTarget = useRestoreFocusTarget();
  const load = useCallback(async () => { if (!clientId) return; setLoading(true); setError(""); try { const [nextSummary, people] = await Promise.allSettled([api.practiceClientSummary(context, clientId), api.resourceProfiles(context)]); if (nextSummary.status === "rejected") throw nextSummary.reason; setSummary(nextSummary.value); setResources(people.status === "fulfilled" ? people.value.items : []); } catch (e) { setError(errorText(e)); } finally { setLoading(false); } }, [context, clientId]);
  useEffect(() => { void load(); }, [load]);
  ...
  const update = (changes: Record<string, string>) => { for (const [key, value] of Object.entries(changes)) value ? parameters.set(key, value) : parameters.delete(key); onNavigate?.(`/practice/clients?${parameters}`); };
  const areas: Array<{ value: ClientArea; label: string }> = [
    { value: "overview", label: "Overview" }, { value: "delivery", label: "Delivery" }, { value: "services", label: "Services" }, { value: "collaboration", label: "Collaboration" },
    { value: "documents", label: "Documents" }, { value: "economics", label: "Economics" }, { value: "activity", label: "Activity" }, { value: "details", label: "Details" },
  ];
  const openWork = summary.workItems.filter((item) => !["completed", "cancelled"].includes(item.status));
  const attention = openWork.filter(item => isOverdue(item.due_date, item.status) || ["waiting_on_client", "review"].includes(item.status) || !item.assigned_member_id && !item.assigned_member_name);
  ...
  return <PersistentClientFrame
    ...
    commands={<CommandBar contextualActions={<><Button appearance="subtle" onClick={() => setRequestOpen(true)}>Request from client</Button><Button appearance="subtle" onClick={() => { setCollaborationTab("messages"); update({ area: "collaboration" }); }}>Messages</Button></>}><Button {...addWorkFocusTarget} appearance="primary" onClick={() => setAddingWork(true)}>Add work</Button></CommandBar>}
  ...
  >
    {error && <Failure message={error} retry={load} />}
    {workspace.area === "overview" && <div className="pm-client-overview">
      <section className="pm-client-attention"><header><h2>Attention</h2><span>{attention.length}</span></header>{attention.slice(0, 5).map((work) => <Button key={work.id} appearance="transparent" className="pm-client-attention-row" onClick={() => update({ selected: work.id })}><span><strong>{work.title}</strong><small>{work.service_name || "Service"} · {date(work.due_date)}</small></span><Status value={work.status} /></Button>)}</section>
      <section className="pm-client-commitments"><header><h2>Current commitments</h2><Button appearance="subtle" size="small" onClick={() => update({ area: "delivery" })}>View delivery</Button></header><dl><div><dt>Open work</dt><dd>{openWork.length}</dd></div><div><dt>Overdue</dt><dd>{overdue.length}</dd></div><div><dt>Waiting on client</dt><dd>{openWork.filter((item) => item.status === "waiting_on_client").length}</dd></div><div><dt>Review</dt><dd>{openWork.filter((item) => item.status === "review").length}</dd></div></dl></section>
    </div>}
    {workspace.area === "delivery" && <ClientDelivery work={summary.workItems} onSelect={(id) => update({ selected: id })} />}
    {workspace.area === "services" && <section className="pm-client-section"><header><div><h2>Services</h2><p>Activate a service before planning its delivery.</p></div><Button onClick={() => setAddingService(true)}>Add service</Button></header><Table aria-label="Client services"><TableHeader><TableRow><TableHeaderCell>Service</TableHeaderCell><TableHeaderCell>Frequency</TableHeaderCell><TableHeaderCell>Team</TableHeaderCell><TableHeaderCell>Status</TableHeaderCell></TableRow></TableHeader><TableBody>{summary.services.map((service) => <TableRow key={service.id}><TableCell><strong>{service.service_name || "Service"}</strong></TableCell><TableCell>{service.frequency ? label(service.frequency) : "Not set"}</TableCell><TableCell>{service.responsible_team_id ? "Assigned" : "Unassigned"}</TableCell><TableCell><Status value={service.status} /></TableCell></TableRow>)}</TableBody></Table></section>}
    {workspace.area === "collaboration" && <ClientCollaboration context={context} clientId={clientId} engagementIds={summary.engagements.map((item) => item.id)} embedded initialTab={collaborationTab} />}
    ...
    {workspace.area === "details" && <ClientPermanentFile context={context} organisationId={clientId} onBack={onBack ?? (() => onNavigate?.("/practice/clients"))} onOpenEngagement={engagementId => onOpenLedgerly?.(engagementId, clientId)} />}
    {addingService && <ActivateClientServiceDialog context={context} clientId={clientId!} existing={summary.services} onClose={() => setAddingService(false)} onCreated={async () => { setAddingService(false); await load(); }} />}
    {requestOpen && <PracticeClientRequestDialog context={context} clientId={clientId!} onClose={() => setRequestOpen(false)} onCreated={async () => { setRequestOpen(false); setCollaborationTab("requests"); update({ area: "collaboration" }); await load(); }} />}
