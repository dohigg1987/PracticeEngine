...
import {
  Badge, Button, Checkbox, createTableColumn, DataGrid, DataGridBody, DataGridCell,
  DataGridHeader, DataGridHeaderCell, DataGridRow, Field, Input, Select, Skeleton,
...
function Prospects({ context, onNavigate, routePath }: { context: ApiContext; onNavigate?: (path: string) => void; routePath: string }) {
  ...
  if (selected) return <section className="pm-page"><Button appearance="subtle" onClick={() => { setSelected(null); onNavigate?.("/practice/crm/prospects"); }}>Back to prospects</Button><Head title={selected.display_name} body={`${label(selected.entity_type)} · ${label(selected.status)}`} />{error && <Failure message={error} retry={() => void open(selected)} />}
    <div className="pm-command-bar"><Button appearance="primary" disabled={selected.status === "converted" || selected.status === "archived"} onClick={() => onNavigate?.(`/practice/crm/opportunities/new?prospect=${encodeURIComponent(selected.id)}`)}>Create opportunity</Button></div>
  ...
  return <section className="pm-page"><Head title="Prospects" body="Pre-client relationships that convert into the canonical client master only after controlled acceptance." />{error && <Failure message={error} retry={load} />}
    ...
    <div className="pm-grid-scroll"><Table aria-label="CRM prospects"><TableHeader><TableRow><TableHeaderCell>Prospect</TableHeaderCell><TableHeaderCell>Primary contact</TableHeaderCell><TableHeaderCell>Open opportunities</TableHeaderCell><TableHeaderCell>Owner / team</TableHeaderCell><TableHeaderCell>Source</TableHeaderCell><TableHeaderCell>Last activity</TableHeaderCell><TableHeaderCell>Status</TableHeaderCell></TableRow></TableHeader><TableBody>{items.map((item) => <TableRow key={item.id}><TableCell><Button appearance="transparent" onClick={() => onNavigate ? onNavigate(`/practice/crm/prospects/${item.id}`) : void open(item)}>{item.display_name}</Button><small>{item.legal_name || label(item.entity_type)}</small></TableCell><TableCell>{item.primary_contact_name || "Not set"}{item.primary_contact_email && <small>{item.primary_contact_email}</small>}</TableCell><TableCell>{item.open_opportunities ?? 0}</TableCell><TableCell>{[item.responsible_member_name, item.responsible_team_name].filter(Boolean).join(" · ") || "Unassigned"}</TableCell><TableCell>{item.source || "Not recorded"}</TableCell><TableCell>{date(item.last_activity_at)}</TableCell><TableCell><Status value={item.status} /></TableCell></TableRow>)}</TableBody></Table></div>
  </section>;
}
...
function Opportunities({ context, onOpenQuoteBench, quoteBenchAvailable, routePath, routeSearch, onNavigate }: {
  ...
  if (selected) {
    ...
    return <section className="pm-page"><Button appearance="subtle" onClick={() => onNavigate?.("/practice/crm/opportunities")}>Back to opportunities</Button><Head title={selected.name} body={`${selected.relationship_name || "Relationship not set"} · ${label(selected.stage_key)}`} />
      ...
      <div className="pm-command-bar"><Button appearance="primary" disabled={selected.status !== "open" || permissionDenied || !capabilities.canEdit} onClick={() => setEditing(true)}>Edit opportunity</Button>{selected.prospect_id && <Button onClick={() => onNavigate?.(`/practice/crm/prospects/${selected.prospect_id}`)}>Open prospect</Button>}{clientId && <Button onClick={() => onNavigate?.(`/practice/clients?client=${encodeURIComponent(clientId)}`)}>Open client</Button>}</div>
  ...
  return <section className="pm-page"><Head title="Opportunities" body="Commercial pipeline from qualified relationship to accepted proposal and conversion." />
    {notice && <MessageBar intent="success"><MessageBarBody>{notice}</MessageBarBody></MessageBar>}{error && <Failure message={error} retry={load} />}
    <div className="pm-command-bar"><Button appearance="primary" disabled={permissionDenied || !capabilities.canCreate} onClick={() => onNavigate?.("/practice/crm/opportunities/new")}>New opportunity</Button></div>
    <section className="pm-filters" aria-label="Opportunity filters"><Field label="Search"><Input type="search" value={query} placeholder="Opportunity, relationship or service" onChange={(_, data) => setQuery(data.value)} /></Field><Field label="Stage"><Select value={stageFilter} onChange={(_, data) => setStageFilter(data.value)}><option value="">All stages</option>{opportunityStages.map((value) => <option key={value} value={value}>{label(value)}</option>)}</Select></Field><Field label="Owner or team"><Select value={ownerFilter} onChange={(_, data) => setOwnerFilter(data.value)}><option value="">All owners and teams</option><option value="unassigned">Unassigned</option>{resources.map((resource) => <option key={resource.id} value={resource.id}>{resource.display_name}</option>)}{teams.map((team) => <option key={team.id} value={team.id}>{team.name}</option>)}</Select></Field><Field label="Lifecycle"><Select value={lifecycle} onChange={(_, data) => setLifecycle(data.value)}>{[["active", "Active"], ["won", "Won"], ["lost", "Lost"], ["all", "All"]].map(([value, text]) => <option key={value} value={value}>{text}</option>)}</Select></Field><Button disabled={!query && !stageFilter && !ownerFilter && lifecycle === "active"} onClick={resetFilters}>Clear filters</Button></section>
    <p className="pm-result-count" role="status">{visible.length} {visible.length === 1 ? "opportunity" : "opportunities"}</p>
    {visible.length ? <div className="pm-grid-scroll"><DataGrid items={visible} columns={columns} sortable getRowId={(item) => item.id} aria-label="CRM opportunities"><DataGridHeader><DataGridRow>{({ renderHeaderCell }) => <DataGridHeaderCell>{renderHeaderCell()}</DataGridHeaderCell>}</DataGridRow></DataGridHeader><DataGridBody<CrmOpportunity>>{({ item, rowId }) => <DataGridRow<CrmOpportunity> key={rowId}>{({ renderCell, columnId }) => <DataGridCell>{columnId === "opportunity" ? <Button appearance="transparent" className="pm-open" onClick={() => onNavigate?.(`/practice/crm/opportunities/${item.id}`)}>{renderCell(item)}</Button> : renderCell(item)}</DataGridCell>}</DataGridRow>}</DataGridBody></DataGrid></div> : <div className="pm-empty"><h2>{items.length ? "No matching opportunities" : "No opportunities"}</h2><p>{items.length ? "Clear or adjust the filters to see more of the pipeline." : "Create an opportunity from here or from a prospect record."}</p>{items.length > 0 && <Button onClick={resetFilters}>Clear filters</Button>}</div>}
  </section>;
}
