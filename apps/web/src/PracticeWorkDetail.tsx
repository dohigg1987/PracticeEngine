...
export default function PracticeWorkDetail({ context, workItemId, onBack, onOpenClient }: {
  ...
  return <PageShell className="pd-page">
    <header className="pd-header">
      ...
      <div className="pd-context"><Badge appearance="tint" color={item.status === "cancelled" ? "danger" : isClosed ? "success" : "informative"}>{label(item.status)}</Badge><span><CalendarLtrRegular aria-hidden="true" />Due {formatDate(item.due_date, "not set")}</span><span><Avatar name={item.assigned_member_name || item.assigned_team_name || "Unassigned"} size={20} color="neutral" aria-hidden="true" />{item.assigned_member_name || item.assigned_team_name || "Unassigned"}</span></div>
    </header>
    ...
    {area === "tasks" && <section className="pd-section" aria-label="Tasks">
      ...
      {tasks.length ? <div className="pd-table-scroll" tabIndex={0} role="region" aria-label="Task table"><Table className="pd-task-table" aria-label="Work tasks" role="table"><TableHeader className="pd-task-head"><TableRow><TableHeaderCell className="pd-task-title">Task</TableHeaderCell><TableHeaderCell className="pd-task-owner">Owner / due</TableHeaderCell><TableHeaderCell className="pd-task-status">Status</TableHeaderCell><TableHeaderCell className="pd-task-actions">Action</TableHeaderCell></TableRow></TableHeader><TableBody>{tasks.map(task => <TableRow className="pd-task-row" role="row" key={task.id}>
...
function DeliveryDialog({ action, item, context, resources, onClose, onSaved }: { action: DialogAction; item: DeliveryWork; context: ApiContext; resources: ResourceProfile[]; onClose: () => void; onSaved: (message: string) => Promise<void> }) {
  ...
  return <Dialog open onOpenChange={(_, data) => { if (!data.open && !busy) onClose(); }}><DialogSurface {...restoreFocusSource}><form onSubmit={event => void save(event)}><DialogBody><DialogTitle>{dialogTitle}</DialogTitle><DialogContent className="pd-form">
    ...
    {action.kind !== "review" && action.kind !== "complete" && <Field label={action.kind === "task" ? "Description" : action.kind === "point" ? "Review point" : action.kind === "point-status" ? "Resolution / reason" : "Reason / notes"} required={reasonRequired}><Textarea disabled={busy} maxLength={(action.kind === "reschedule" || action.kind === "stage") ? 500 : action.kind === "task" || action.kind === "decision" ? 2000 : 4000} value={reason} onChange={(_, data) => setReason(data.value)} resize="vertical" /></Field>}
