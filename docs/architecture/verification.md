...
- The authoritative pilot browser stage defaults to two process shards per detected browser. With Edge installed that is four jobs—Chromium 1/2 and 2/2 plus Edge 1/2 and 2/2—scheduled in two batches with at most two jobs concurrent by default. `PLAYWRIGHT_SHARDS_PER_BROWSER` may override the positive shard count, and `PLAYWRIGHT_BROWSER_JOB_CONCURRENCY` may override the positive concurrency bound.
