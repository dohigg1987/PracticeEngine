...
The fourth focused run passed all eight recovery scenarios and `verify:integration`, confirming the redirect assertions needed to allow full-page session restoration. Its pilot invocation rejected the one-worker override because the pilot coordinator requires at least two workers for its shard jobs. The override is now scoped to the unsharded `verify` command; pilot retains its default four-worker budget. No production code or test assertions changed in this correction.
